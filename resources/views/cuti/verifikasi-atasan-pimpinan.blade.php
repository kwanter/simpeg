<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Verifikasi Atasan Pimpinan Permohonan Cuti') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="card">
                <div class="card-header bg-light">
                    <h5 class="card-title mb-0">Form Verifikasi Atasan Pimpinan</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('cuti.proses-verifikasi-atasan-pimpinan', $cuti->uuid) }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status_atasan_pimpinan" id="status_disetujui" value="Disetujui" required>
                                    <label class="form-check-label" for="status_disetujui">
                                        Disetujui
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status_atasan_pimpinan" id="status_ditolak" value="Ditolak" required>
                                    <label class="form-check-label" for="status_ditolak">
                                        Ditolak
                                    </label>
                                </div>
                            </div>
                            @error('status_atasan_pimpinan')
                                <div class="text-danger mt-1">{{ $message }}</div>
                            @enderror
                        </div>

                        <div class="mb-3">
                            <label for="catatan_atasan_pimpinan" class="form-label">Catatan Atasan Pimpinan</label>
                            <textarea name="catatan_atasan_pimpinan" id="catatan_atasan_pimpinan" rows="3" class="form-control"></textarea>
                            @error('catatan_atasan_pimpinan')
                                <div class="text-danger mt-1">{{ $message }}</div>
                            @enderror
                        </div>
